...
<div>
    ...
    <form action="./api/edit_vote.php" method="post">
        <table class="edit-table">
            ...
            <tr>
                ...
                <td class="edit-td2"><input type="date" name="start"
                        value="<?=date('Y-m-d',strtotime($subj['start']))?>"></td>
            </tr>
            <tr>
                <td class="edit-td1">結束時間：</td>
                <td class="edit-td2"><input type="date" name="end" value="<?=date('Y-m-d',strtotime($subj['end']))?>">
                </td>
            </tr>
            <tr>
                <td class="edit-td1">顯示前台</td>
                <td class="edit-td2"><input type="radio" name="sh" value="1" <?=($subj['sh']==1)?'checked':'';?>>是<input
                        type="radio" name="sh" value="0" <?=($subj['sh']==0)?'checked':'';?>>否</td>
            </tr>
            <tr>
                <td></td>
                <td class="edit-td2" style="text-align: center;"><input type="button" value="新增選項" id="addvote"
                        required></td>
            </tr>
            <!-- 使用表格群組tbody -->
            <tbody id="options">
    ...
    $("#addvote").click(function() {
        $("#options").append('<tr id="tr' + trId +
            '"><td class="edit-td1"><label>選項:</label></td><td class="edit-td2"><input type="text" name="option[]" required><button class="editbut" type="button" onclick="delbtn(' +
